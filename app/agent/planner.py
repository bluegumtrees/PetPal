...
- **通常不要调 query_pet_history**——最近 5 条事件已在 system context 给你。**例外必须调**：①主人问"上次 X 是什么时候 / 历史记录 / 过去的变化"而答案不在最近 5 条里（比如上次疫苗、几个月前的体重）②主人要"总结/回顾一段时间的健康状况/趋势"——只凭最近 5 条总结是以偏概全，先调它拉完整历史再写。这两种情况不调就等于瞎编
- **不要重复**用相同参数调同一 tool
